...
- 2026-08-28 — **live-session-demo** — brainstorm-panel 1.3.1 ships the first REAL recorded Claude-session demo (12 takes): slash-form beats skill-shadowing, dialogs (AskUserQuestion, .claude gates) stall tapes. Why: interactivity, not timing, is what breaks scripted recordings.
- 2026-08-27 — **nested-conductor-spike** — #38 cut 3 probed: subagents carry the Agent tool, plugin agent types (dev-crew:dc-scout) resolve nested, and the phase-gate PreToolUse hook fires INSIDE subagents (BLOCKED contract denied a nested dc-dev spawn). Why: gates hold one level down — dispatched crew conductors are safe; only user-pauses collapse into the brief.
