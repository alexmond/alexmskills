...
| Memory it guards
| Claude's native project memory under `~/.claude/projects/<slug>/memory/`.
| Project-owned `.codex/memory/MEMORY.md` plus topic files — plugin-managed, not Codex's internal memory database.

| Hooks
| SessionStart audit, PreToolUse format lint on memory writes.
| The same two events. SessionStart also loads the memory index into context.

| Direct CLI audit
| Runs as-is.
| Set `SKILL_CLIENT=codex` so the audit reads `.codex/memory/`.

|===

*The difference that matters:* The Codex memory is files this plugin manages. Anything Codex remembers on its own is outside its reach.
