...
This is informational, not praise — a heartbeat that also shows your progress toward the next mastery. Rules graduate to "mastered" after `graduation_threshold` clean prompts in a row (default 15) *and* an evidence gate (`min_fires_for_mastery`), then the next dormant rule activates. When a rule masters you get a `🎓` congratulation.
...
Mastered rules aren't permanently dormant. They still evaluate every prompt, and when only a mastered rule matches, a *refresher* fires — a lighter re-fire rather than the full coaching block. The cooldown for mastered rules is much longer (50 prompts vs 5 for practicing rules). Mastery is evidence-based: a rule only masters if it has actually fired at least `min_fires_for_mastery` times; a rule that reaches the clean-streak threshold with zero fires retires as *inactive* (it never applied to how you prompt) rather than claiming a mastery you never earned.
