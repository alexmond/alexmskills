...
| `--ai-client codex` runs Codex in a read-only sandbox with hooks, plugins, apps, web search and subagents disabled; `auto` picks Codex inside Codex.

| MCP servers
| The Claude runner's normal configuration.
| Every configured MCP server is disabled for the call; if the configuration can't be inspected, expansion stops before any model request.

| Commands
| `/mindmap-prompt:*`.
| Ask in plain language; start `scripts/serve.py --ai-client codex`.

|===

*The difference that matters:* Codex expansion is deliberately sealed off from project tooling; `--ai-read` opens local read-only exploration when you want it.
