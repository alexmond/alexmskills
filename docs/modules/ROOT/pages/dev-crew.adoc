...
See xref:role-system.adoc[The Role System] for how crew roles are shared and evolve, and
xref:role-system-usage.adoc#usage-sequence[Choosing & chaining] for how to chain it with research and panel.
...
* *Parallel work — compose, don't multiplex.* The relay stays sequential (its phases are a dependency chain), but parallelism now has a documented home at two other levels: *across tickets*, one relay per ticket in its own git worktree, scheduled by the xref:ticket-triage.adoc[ticket-triage] plugin — the crew's run state is untracked and CWD-relative, so concurrent relays isolate for free and merges back to main are the scheduler's serialization point; *within a ticket*, the `tech-lead` candidate role fans out parallel `dev` subagents over independent work items and integration-reviews the merge before qa. One conductor interleaving pipelined relays is the documented fallback, not the design.
...
| Roles
| `agents/dc-*.md` register as subagent types with their own tool lists and model tiers.
| The full role file is passed in each child's brief via `spawn_agent`; Markdown agents are not registered Codex agent types.

| Phase gate
| PreToolUse hook blocks an out-of-order Agent dispatch.
| The same gate runs on spawns — the exact `dc-*` role name must appear in the task name and message, or the gate can't identify it.

| Tool limits
| Enforced by the runtime from agent metadata.
| Stated in the brief — an instruction, not a sandbox.

|===

*The difference that matters:* On Codex a role told "read-only" is trusted to comply rather than prevented from writing. Model preferences map to models the client actually offers.
