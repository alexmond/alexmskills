...
=== Tap: measured bars for Maven and git (0.5.0)

`progress_tap.py` is a transparent pipe filter (generalized from the production tap in a large multi-module project's build gate): stdin passes through byte-for-byte, and the position lines the tool already prints — Maven's reactor `[3/15]`, git's `Receiving objects: 42% (12345/29292)` — become a *measured* `items` bar rather than a time estimate:
...
Patterns: `maven` (default), `git`, `batch`, `count:<regex>` with `--total`. Read `${PIPESTATUS[0]}` for the build's verdict (a pipeline's `$?` is the tap's), and pass `--progress` to git in a pipe. Daemon down? It degrades to `cat`, silently — a bar is never worth a broken build. The advisory nudge suggests the tap automatically for Maven/Gradle/make and long git operations (`clone`, `fetch`, `pull`, `lfs`, `submodule`).
...
| Live bars
| In the Claude Code status line (`statusLine` + `refreshInterval`).
| In the browser dashboard or `progress watch` in a companion terminal. Codex has no status line.

| Hooks
| SessionStart what's-new notice, PreToolUse long-command nudge.
| The same two, via `hooks/codex.json`. The nudge never approves a command.

| Session scoping
| `CLAUDE_CODE_SESSION_ID`.
| `PROGRESS_SESSION_ID`, then Claude's id, then `CODEX_THREAD_ID` / `CODEX_SESSION_ID`.
