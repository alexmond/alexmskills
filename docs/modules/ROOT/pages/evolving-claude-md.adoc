...
* *Merge as the fourth downward pressure* — SKILL.md now documents merging same-session same-area clusters (phased rollouts `e2a`/`e2b`/`e3`/`e4`; multi-aspect bursts within ~48h) as the compaction lever that works *pre-14-days*. Graduation and quarterly archive both require old entries; merge does not. Discovered while compacting jvmlens's 37-entry log when the audit fired "RECOMMENDED" but every documented pressure was blocked.
