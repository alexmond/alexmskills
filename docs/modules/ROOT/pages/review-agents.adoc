...
The Claude Code workflow remains available in full. Codex uses the client
reference shipped with the skill for instruction files, tools, and learning
state. See xref:codex.adoc[Codex compatibility] for hook setup and runtime
limits. Agent roles retain their complete definitions when dispatched on Codex.
