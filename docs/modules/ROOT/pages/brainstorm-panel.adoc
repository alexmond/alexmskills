...
See xref:role-system.adoc[The Role System] for how panel seats are shared with the crew and evolve, and
xref:role-system-usage.adoc#usage-sequence[Choosing & chaining] for how to chain it with research and crew.

TIP: Watch it live: the repo ships a
https://github.com/alexmond/alexmskills/blob/main/plugins/brainstorm-panel/demo/demo.gif[26-second demo GIF]
of a real recorded session — roster gate, `go`, parallel seats, verdict — with its
https://github.com/alexmond/alexmskills/blob/main/plugins/brainstorm-panel/demo/demo.tape[VHS tape source]
committed alongside.
...
* *Unanimity guard now leaves evidence.* An audit across 47 panel runs in 5 sibling repos found that 30% were round-1 unanimous but the 1.1.0 anti-groupthink guard (forced steelman) had fired 0 times in the logs — silent-by-design, indistinguishable from groupthink from outside. The Step 6 log template now requires a `Steelman:` field whenever R1 was unanimous, with one of: `ran, opposing view was X — retained | adopted | panel reversed` / `waived because <reason>` / `n/a — pushback present in R1`. Spec prose reframes unanimity as a risk signal, not a celebration. Closes #25.
* *Anti-anchoring* (seats receive only the question in R1, no prior synthesis from director or other seats — `affaan-m/ECC council` mechanism) is *deferred*, contingent on a revalidation pass after 10+ new runs accumulate with the auditable guard. Cheapest-fix-first.
...
* *Per-seat model tiers + gated Fable escalation* (issue #8) — registry rows in `.claude/roles/panel.md` now carry a `model` field (the same field as crew.md), so a tier decision made once is durable instead of a per-run ask. Fable is never auto-selected: the panel is generation-bound, so it is proposed at the roster checkpoint only, for a *single* deep/wildcard generator seat (optionally the skeptic) with the ~2× Opus cost stated — Fable on every seat would flatten exactly the diversity the panel exists to create. (This re-arms what 1.1.2 had withdrawn during the 2026 Fable suspension; access has since been restored.)
...
* *Fable suggestion removed* — Anthropic suspended Claude Fable 5 / Mythos 5 on 2026-06-12 in response to
  ...
  withdrawn until access is restored. (Access was later restored; see 1.3.0 above — issue #8 shipped.)
...
| Seats
| Each seat is a parallel subagent.
| Each seat is a `spawn_agent` child (or the client's live equivalent); enable multi-agent where supported.

| Roster gate and learning
| Propose seats, wait for approval, record outcomes.
| Unchanged — conversation beats and plain files.

|===

*The difference that matters:* Without agent tools the panel runs its seats one at a time in one context and must say so, because independent critique is part of what it promises.
