...
xref:role-system-usage.adoc#usage-sequence[Choosing & chaining] for how to chain it with panel and crew.
...
* *Per-role model tiers + gated Fable escalation* (issue #8) — `.claude/roles/research.md` rows gain a `model` field; the sweep is verification-bound, so Fable is proposed (never auto-selected) at the roster gate for the skeptic-verifier and optionally the synthesizer, cost stated — the parallel scouts stay on cheap tiers, where breadth per token is the whole point. (Re-arms what was withdrawn during the 2026 Fable suspension.)
...
* *Fable suggestion removed* — Anthropic suspended Claude Fable 5 / Mythos 5 on 2026-06-12 in response to
  ...
  withdrawn until access is restored. (Access was later restored; see 1.2.0 above — issue #8 shipped.)
...
| Scouts
| Parallel subagents on disjoint slices.
| `spawn_agent` children — issue together, collect with one wait.

| Extraction
| `jq` pulls each scout's YAML from Claude subagent transcripts.
| Collect YAML from each child's returned message. Do not read transcript files.

|===

*The difference that matters:* The fan-out and verification method are unchanged. Only the step that harvests results differs, because Codex keeps child output elsewhere.
