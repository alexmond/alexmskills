...
* One issue at a time by design. To drain a whole backlog in parallel — ranked, worktree-isolated, merged as each agent finishes — reach for xref:ticket-triage.adoc[ticket-triage].
...
| Attribution
| Claude generated-by trailers.
| Attribute to the client actually used — no Claude identity, and no invented Codex co-author email.

|===

*The difference that matters:* Behaviour is the same; only the attribution line changes.
