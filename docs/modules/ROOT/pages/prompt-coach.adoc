...
A `UserPromptSubmit` hook that watches every prompt you send Claude Code or Codex and coaches you toward better prompting habits — definition-of-done, scoped references, guardrails, verification, and more. As of v0.34 the coach is a *collaborator*: when a rule fires, Claude reads your prompt in context and rewrites it with the fix baked in, at the top of its response. Rules quietly graduate once you consistently apply them, so the coach fades as your prompts improve.
...
Version 1.3.0 adds native Codex `UserPromptSubmit` hooks (baseline CLI 0.154.0).
Install the plugin through the xref:codex.adoc[Codex marketplace], start a new
session, then use `/hooks` to review and trust its hook definition. Changed
hook definitions require another trust review. Do not also register the same
analyzer as a user hook: both registrations would count each prompt.

The assistant renders coaching from injected context; no custom status bar is
needed. Both clients intentionally share the existing `~/.claude/prompt-coach`
and repo `.claude/prompt-coach` state. Claude Code need not be installed.
Concurrent sessions still share the per-repo acceptance window; use separate
worktrees for separate local state.

The supplied transcript path takes precedence over legacy Claude discovery.
Codex assistant messages and question-tool calls are normalized; missing or
unknown transcript formats fall back to prompt-only analysis, never to another
client's transcript. Codex transcript formats are not a stable API.

`make test-coach` runs the shared regression suite and client contract tests.
`make test-coach-codex` additionally runs the installed Codex CLI against a
local mock model server, verifying that the hook executes and its coaching
reaches the model request. It uses temporary config/state and no model tokens;
the test's hook-trust bypass applies only to its isolated, reviewed test hook.
...
The catalog ships **42 rules** across 6 tiers (L1 6 · L2 4 · L3 11 · L4 5 · L5 11 · L6 5), each with a mirroring **positive detector** (42 total) so mastery is *earned by demonstration*. Behavior is tuned by **40 configuration keys** (see xref:prompt-coach-config.adoc#config-reference[the full table]).
...
| Hook
| `UserPromptSubmit` from `hooks/hooks.json`.
| Native `UserPromptSubmit` from `hooks/codex.json` (baseline Codex CLI 0.154). Trust with `/hooks`; a changed definition needs another review.

| State
| `~/.claude/prompt-coach` and the repo's `.claude/prompt-coach`.
| The same paths, deliberately — mastery, config and history carry across clients.

| Transcripts
| Claude transcripts drive clarification and rejection timing.
| Codex rollout items are normalized for the same signals; an unknown format degrades to prompt-only analysis.

| Commands
| `/prompt-coach:stats`, `:config`, `:dashboard`, …
| Ask in plain language; the same `commands/<verb>.md` procedure runs.

|===

*The difference that matters:* Register once per client: a user hook next to the plugin hook analyzes and counts every prompt twice. Codex transcripts are not a stable API, so fixtures need refreshing when they change.
