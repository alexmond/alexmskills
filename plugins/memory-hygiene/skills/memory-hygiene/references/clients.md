...
Use project-owned `.codex/memory/MEMORY.md` and topic files beside it. This is
plugin-managed memory, not Codex's internal memory database. The Codex SessionStart
hook loads the index; read relevant linked files on demand. Direct audits require
`SKILL_CLIENT=codex`. Preserve the same metadata, freshness checks, and index rules.
