...
- **name** · one-line **charter** · **when-to-cover** (the trigger / angle that earns it a place — the information-space signal it owns) · **model** (the tier the role runs on — parallel scouts stay on cheap tiers; the verifier and synthesizer earn heavier ones; same field as crew.md/panel.md so tier decisions are durable) · **status** (probationary → stable once it has proven useful on three or more runs; demoted to probationary on two thin runs with no fix; retired after three thin runs or three user-cut events at the roster gate) · **learnings** (accumulated bullets: which corpora it paid off on, dedup gotchas, source-trust rules, verification failure modes).
...
**Gated Fable escalation (mirrored from dev-crew — never auto-selected).** The sweep is *verification-bound*: its failure mode is trusting a plausible-but-wrong synthesis, so Fable spend belongs on the **skeptic-verifier** (and optionally the synthesizer) — never on the parallel scouts, whose value is breadth per token. Propose it at the roster gate only, with the cost stated (~2× Opus); an accepted escalation is written to the role's `model` field so it persists. Three ways to run a role on Fable: its registry row's `model`; `CLAUDE_CODE_SUBAGENT_MODEL` for the run; or the whole session on Fable. Spawn each role's Task on its row's tier.
