...
Start `scripts/serve.py --ai-client codex` from the plugin root (with `--cwd`
pointing at the consuming repo). `auto` selects Codex inside Codex sessions,
otherwise Claude when installed, then Codex. `--ai-client claude` forces the
original runner. `--ai-model` applies to the selected client; `--ai-read` permits
local exploration. Codex runs with a read-only local sandbox; hooks, plugins,
apps, automatic MCP installation, web search, and subagents are disabled. It
inspects effective MCP configuration in the consuming project and disables every
listed server for this invocation, retaining the user's model/provider/auth
settings. If inspection fails, expansion stops before making a model request.
The shell is disabled unless `--ai-read` is selected. The answer
comes from `--output-last-message`; canvas previews and explicit apply stay intact.
