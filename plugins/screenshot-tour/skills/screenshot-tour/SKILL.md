...
**Pre-check every aspect for an affordance trap.** A shot that renders a
setup prompt ("requires an AI model", "no data yet") or a thin demo dataset
captures the *affordance*, not the *payoff* — and quietly weakens the deck.
For each aspect, ask: *what will actually render if I capture this right
now?* When the answer is a prompt or an empty state, say so in `plan.md` on
that aspect's line — e.g. *"renders the setup prompt unless a completed
result is seeded first"* or *"demo dataset has 6 rows; point at the richer
dataset for more impact"* — so the user can opt into seeding before capture
instead of discovering the weak shot in the finished deck.
...
For each aspect in `plan.md`, run the matching capture method. Each method ships its own recipe file under `presentation/recipes/` so the tour is re-runnable on the next release. When a project's own screenshot tooling does the capturing, the recipe is a one-line wrapper that invokes it — **never leave `recipes/` empty or absent while the repro command lives only in prose**; an empty dir tells the next runner the tour isn't re-runnable when it is.
...
- **Host hygiene on every surface, not just shell recipes.** Any URL that will
  be *rendered into pixels* — a Freeze'd CLI transcript, a VHS `Type` line, a
  visible browser address bar — goes through the same substitution as the
  recipes: the project's documented public URL, or a neutral placeholder like
  `https://example.invalid`, never whatever instance the capture ran against.
  A hostname baked into a committed PNG is invisible to every text-based
  secret scanner — parameterizing only the recipe was the miss behind this rule.
...
**Lead with:** 01-hero · 03-<core-flow> · NN-outcome <!-- the 3-shot subset
for a skim, a forum post, or a social embed — pick the images that carry the
story alone -->
...
Three template rules, pinned after real decks diverged on each:

- **The attribution link is canonical** — exactly the URL above, always; two
  URLs for one skill reads as two tools.
- **The repro command lives in the footer, only** — never duplicated into the
  header note.
- **Every deck marks the "Lead with:" 3-shot subset** — a skim or a forum post
  gets one or two images; say which carry the story.
