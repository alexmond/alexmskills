...
- One lane per ticket, one worktree per lane — unchanged; the isolation is git's,
  not the harness's.
- Dispatch a lane with `spawn_agent`, correct one with `followup_task` (never
  re-spawn a fresh worker to deliver a correction), and reconcile with
  `list_agents` after each wait stretch.
- Honest width still comes from the machine, not the tool: measure the ceiling
  and hold it.
