...
|---|---|---|---|
| [`evolving-claude-md`](plugins/evolving-claude-md) | self-learning | 1.6.1 | Turns CLAUDE.md into a living Decisions & Learnings log that prunes, graduates, and archives itself via hooks — with a bar at the inlet (a delivery is not a learning), tree-grounded coverage, capture triggers for the never-written-to log, obsolescence findings that name the edit, a per-lane spool so parallel agents stop colliding on one file, and a researched structure-review rulebook. |
