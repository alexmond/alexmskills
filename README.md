...
| [`mindmap-prompt`](plugins/mindmap-prompt) | workflow | 0.1.2 | Sketch ideas on an offline canvas, connect them, and compile the map into an organized Markdown prompt. Saves as JSON Canvas (`.canvas`) so maps are git-diffable, reworkable, and open in Obsidian. |
